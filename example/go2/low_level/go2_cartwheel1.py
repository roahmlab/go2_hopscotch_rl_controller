...
class Custom:
    def __init__(self):
        ...
        f = np.load(os.path.join(base_dir, "hopscotch_utils", "trajectories_cartwheel_v6.npz"))
        ...
        self.blind = "actor_cartwheel1.pkl"
        ...
        self.use_quat = obs_tag == "quat_v5_pos"
        self.use_mocap = obs_tag in ("euler_v3_pos", "quat_v5_pos")
        obs_width = (73 + 2 * self.use_quat + 3 * self.use_accelerometer
                     + 3 * self.use_mocap
                     + (30 + self.use_quat) * len(self.preview_offsets))
        if self.arch == "gru":
        ...
        self.obs_quat_prev = np.array([1.0, 0.0, 0.0, 0.0])
        self.fault = False
        self.gyro_alpha = 1.0
        self.gyro_f = None
    ...
    def LowCmdWrite(self):
        ...
        if self.fold_percent < 1:
            ...
            # hemisphere continuity: a raw estimator sign flip would step every quat
            # feature mid-flip (the reference side is continuous by construction)
            if np.dot(base_quat, self.obs_quat_prev) < 0.0:
                base_quat = -base_quat
            self.obs_quat_prev = base_quat
            ...
            st = np.concatenate([base_quat, dof_pos, ang_vel_body, dof_vel])
            qerr = quat_mul(quat_inv(xr[3:7]), base_quat)
            err = np.concatenate([qerr, xr[7:19] - dof_pos,
                                  xr[22:25] - ang_vel_body, xr[25:37] - dof_vel])
