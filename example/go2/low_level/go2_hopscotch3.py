...
class Custom:
    def __init__(self):
        ...
        self.foot_clr = np.full(4, np.nan)
    ...
    def LowCmdWrite(self):
        ...
        if self.fold_percent < 1:
            ...
            if self.use_feet:
                # p_meas is anchored to the RAW x_ref[0] (point-foot solve) while the trainer
                # raised its reference by the foot radius, so p_meas sits exactly FOOT_R below
                # true world height -- the radius cancels and clr = p_meas_z + (R @ p_foot)_z.
                pf = feet_in_base(dof_pos)
                self.foot_clr = np.array(
