...
class HopscotchRef:
    ...
    def __init__(self, json_path):
        ...
        #ffcomp for model trained with sys-id'd params
        FF_D = [3.7e-8, 0.0982,0.3483] #viscosity, hip-thigh-calf
        FF_IA = [0.00433, 0.00433, 0.01591]  # rotor armature
        d12  = np.array([FF_D[0] if "hip" in n else FF_D[1] if "thigh" in n else FF_D[2]
                 for n in ISO_NAMES])
        ia12 = np.array([FF_IA[0] if "hip" in n else FF_IA[1] if "thigh" in n else FF_IA[2]
                 for n in ISO_NAMES])
        
        # (comp applied below — it needs gather / qd_ref / tau_ff to exist first)
        ...
        self.tau_ff = u[:, gather]
        # sysid ff comp, mirroring the training bake (reference_manager_hopscotch.py):
        # u <- u + d*qd_ref + Ia*qdd_ref. The MDC back-EMF envelope clamp that training
        # applies after this is a NO-OP at these comps on the lsq traj (0.00% of knots
        # clip, 0.72 N-m of headroom at closest approach), so it is omitted here.
        qdd_ref = np.concatenate(as_)[:, 6:18][:, gather]   # 'a', half-open like q/v/u
        self.tau_ff = self.tau_ff + d12 * self.qd_ref + ia12 * qdd_ref
