"""Go2 hardware deployment for the CARTWHEEL (cart / kdm_v2s_retarget family).

Sibling of deploy_dm.py, which flies the kin_v4 dm/hopscotch line and is kept
untouched. The CONTROLLER is identical and copied verbatim -- q* = anchor +
a*action_scale latch-held at 50 Hz, qd* per kin_anchor_qd, no tau_ff, 58-wide
obs frames, obs_tau_mode "cmd", 1-step sensor+action latency, the same stand-up
fold/align/hold + integrator handoff. kdm_v2s_retarget's meta matches that
contract exactly (kinematic, kin_anchor home, action_scale 1.7, act_lpf_beta
0.7, ff comps 0.0, obs_velest False), so nothing in the control path changes.

TWO THINGS ARE DIFFERENT, AND BOTH WOULD COST THE RUN ON deploy_dm.py:

1. THE BASE EULER CHAIN IS NOT XYZ.  deploy_dm hardcodes
       base_quat = euler_xyz_to_quat_wxyz(q[:, 3:6])
   but the cartwheel archive is go2_yxz: base_ry -> base_rx -> base_rz.  Reading
   it as XYZ garbles the whole INVERTED section by up to 180 deg -- and does so
   SILENTLY, because the misread leaves grav_z = cos*cos identical, so every
   gravity-based check still looks fine while obs_ori_err and yaw_e are wrong
   through exactly the part of the clip that matters (the 2026-08-28 cart_v0
   post-mortem; reference_manager_hopscotch.py asserts against it).  This script
   reads the chain from the file's OWN joint_names[3:6] and composes in chain
   order, and REFUSES to guess if the names are unrecognised.

2. THE ABSOLUTE TILT ABORT IS INVALID FOR THIS MOTION.  deploy_dm aborts on
       grav_b[2] > -0.4
   i.e. "more than ~66 deg from upright".  The cartwheel's own reference reaches
   108 deg of tilt and passes through a nose-down handstand, so that guard fires
   DURING THE MOUNT, before the trick starts.  A looser constant is not the fix:
   the motion is legitimately inverted, so the only meaningful question is
   whether the robot is where the REFERENCE says it should be.  This script
   aborts on the GEODESIC angle between measured and reference attitude
   (rotation-honest, frame-free, valid through the inversion -- the same rebuild
   cart_metrics.py needed), with the legacy absolute floor re-armed only while
   the reference itself is upright.

SAFETY, READ THIS BEFORE FLYING:
  The checkpoint this was written for (kdm_v2s_retarget/model_7400) lands in
  simulation only at EXACTLY nominal foot friction -- measured basin is 2.5 %
  wide (mu 0.78 fails, 0.80 lands, 0.82 fails), and 0/32 randomised plants
  complete the task.  What IS robust across every plant tested is the mount
  (handstand 100 %) and the flight rotation (75-94 %).  Fly this expecting the
  CATCH TO FAIL and the dog to end up on the mat; the value is the mount/flight
  data, the contact trace at touchdown, and derisking this pipeline.  Harness
  and mats are not optional here.

Usage:
    python3 deploy_cart.py --checkpoint runs/kdm_v2s_retarget/model_7400.pt [iface]
    # meta.json is auto-found next to the checkpoint; --traj overrides meta's
    python3 deploy_cart.py --checkpoint ... --dry-run     # load + assert, no robot
"""
import time
import sys
import os
...
CTRL_DT = 0.02
# Abort policy (see the module docstring). ORI is the real guard; the absolute
# floor below is only armed while the REFERENCE attitude is upright.
ABORT_ORI_DEG_DEFAULT = 60.0
REF_UPRIGHT_GRAV_Z = -0.70
...
_BASE_AXIS = {"base_rx": 0, "base_ry": 1, "base_rz": 2}
_BASE_XYZ = ("base_rx", "base_ry", "base_rz")


def euler_chain_to_quat_wxyz(e, base_names):
    """Intrinsic euler in the archive's OWN base-joint order -> quat wxyz.

    R = R_a1(e1) R_a2(e2) R_a3(e3) with the axes read from base_names. cart.npz
    is go2_yxz (base_ry -> base_rx -> base_rz); composing it as XYZ is the
    cart_v0 bug -- silent, because grav_z is unchanged by the misread.
    """
    bad = [n for n in base_names if n not in _BASE_AXIS]
    if bad or len(base_names) != 3:
        raise ValueError(
            f"unrecognised base joint names {list(base_names)} -- refusing to guess "
            f"the euler chain. Expected a permutation of {list(_BASE_AXIS)}.")
    ...
    def qmul_batch(a, b):
        ...
        return np.stack([aw * bw - ax * bx - ay * by - az * bz,
                         ...
                         aw * bz + ax * by - ay * bx + az * bw], axis=1)
    e = np.atleast_2d(np.asarray(e, float))
    qs = [axis_quat(e[:, i], _BASE_AXIS[n]) for i, n in enumerate(base_names)]
    return qmul_batch(qmul_batch(qs[0], qs[1]), qs[2])


def geodesic_deg(qa, qb):
    """Angle of the relative rotation between two wxyz quats, in degrees. Sign- and
    frame-free: valid through the handstand where euler differences are not."""
    d = min(1.0, abs(float(np.dot(np.asarray(qa, float), np.asarray(qb, float)))))
...
class HopscotchRef:
    ...
    def __init__(self, path):
        if str(path).endswith(".npz"):
            q, v, contact, force_ref, src_names, base_names, dt = self._load_npz(path)
            fmt = "gridded npz"
        else:
            q, v, contact, force_ref, src_names, base_names, dt = self._load_json(path)
        ...
        self.base_euler_chain = tuple(base_names)
        self.base_quat = euler_chain_to_quat_wxyz(q[:, 3:6], self.base_euler_chain)
        if self.base_euler_chain != _BASE_XYZ:
            logging.warning("*** NON-XYZ base euler chain %s (from the archive's own "
                            "joint_names) -- composing in CHAIN order. Reading this as "
                            "XYZ garbles the inverted section SILENTLY. ***",
                            "->".join(n[-2:] for n in self.base_euler_chain))
    ...
    def _load_json(path):
        ...
        base = list(modes[0]["joint_names"][3:6])
        return (np.concatenate(qs), np.concatenate(vs), np.concatenate(cs),
                np.concatenate(fs), src, base, float(modes[0]["dt"]))
    ...
    def _load_npz(path):
        ...
        base = [str(x) for x in d["joint_names"][3:6]]          # THE CHAIN, by name
        return q, v, contact, force, src, base, 1.0 / float(d["rate"])
...
class Custom:
    def __init__(self, ckpt_path, meta, traj_override=None, meta_path=None):
        ...
        self.aborted = False
        self.abort_ori_deg = ABORT_ORI_DEG_DEFAULT   # overridden from argv in main
        self._ori_err_deg = float("nan")
    ...
    def _policy_tick(self):
        ...
        if self.motiontime % 10 == 0:
            ...
            logging.info("t %.2f cont %s%s rp_e [%+.1f %+.1f] yaw_e %+.1f tilt %.2f "
                         "ori_e %.0f |a| %.2f (loop %.1f Hz)",
                         t, contacts_true.astype(int), " (blinded)" if self.blind else "",
                         *_rp, np.degrees(yaw_e), grav_b[2],
                         getattr(self, "_ori_err_deg", float("nan")),
                         np.abs(a_cmd).max(), self._loop_hz)

        # ---- ABORT: deviation from the REFERENCE attitude, not from upright -----
        # deploy_dm's `grav_b[2] > -0.4` is invalid here: this motion is legitimately
        # inverted (ref max tilt 108 deg, through a nose-down handstand), so that guard
        # fires during the mount. The rotation-honest question is whether the robot is
        # where the reference says. The legacy absolute floor is re-armed ONLY while the
        # reference itself is upright, so a plain fall still trips it.
        ori_err = geodesic_deg(quat, quat_ref)
        grav_ref_z = float((rotmat_from_quat_wxyz(quat_ref).T
                            @ np.array([0.0, 0.0, -1.0]))[2])
        self._ori_err_deg = ori_err
        if ori_err > self.abort_ori_deg:
            logging.error("ORI ABORT at t %.2f: %.0f deg off the reference attitude "
...
if __name__ == '__main__':
    ...
    ap.add_argument("--checkpoint", default = "dm_utils/kpw22_850.pt",required=False,
                    help=".pt of a KINEMATIC-family run (dm_v*, kin_v4+)")
    ap.add_argument("--meta", default="dm_utils/kpw22_meta.json", help="default: meta.json beside the checkpoint")
    ap.add_argument("--traj", default=None,
                    help="override meta's traj_path (default: whatever the ckpt was "
                         "trained on, e.g. cart_retarget_v2.npz)")
    ap.add_argument("--abort_ori_deg", type=float, default=ABORT_ORI_DEG_DEFAULT,
                    help="abort when the measured attitude is this far (geodesic) from "
                         "the reference attitude. NOT an upright test -- this motion "
                         "inverts by design. Lower = safer/earlier cut.")
    ...
    if args.dry_run:
        ...
        c.abort_ori_deg = float(args.abort_ori_deg)
        ...
        logging.info("base euler chain: %s | abort: ori>%.0f deg, or grav_z>-0.40 "
                     "while the reference is upright",
                     "->".join(n[-2:] for n in c.ref.base_euler_chain),
                     c.abort_ori_deg)
    ...
    custom.abort_ori_deg = float(args.abort_ori_deg)
    logging.info("base euler chain: %s | abort: ori>%.0f deg, or grav_z>-0.40 while "
                 "the reference is upright",
                 "->".join(n[-2:] for n in custom.ref.base_euler_chain),
                 custom.abort_ori_deg)
    custom.Init()
    custom.Start()
    ...
    try:
        ...
        # CTRL-C IS THE SOFT E-STOP -- ORDER MATTERS.
        # KeyboardInterrupt lands in the MAIN thread only; the 50 Hz control loop is a
        # separate daemon RecurrentThread that keeps publishing POLICY output until the
        # process actually exits. deploy_dm rendered the trace (matplotlib PNGs, seconds)
        # BEFORE re-raising, so Ctrl-C on a thrashing dog bought several more seconds of
        # thrashing. Damp first: setting `aborted` is picked up by the next control tick
        # (<=20 ms), which publishes kp=0 kd=3 tau=0 and keeps publishing it. Only then
        # spend time on artifacts.
        custom.aborted = True
        time.sleep(0.25)                      # >= 12 damped ticks on the wire
        logging.warning("Ctrl-C -> damped stop published (kp=0 kd=3); saving trace")
