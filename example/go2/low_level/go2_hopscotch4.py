...
# record_floating_base.py --feet: base pose then four foot xyz, its own magic so the
# two layouts can never be confused. Feet arrive NaN when occluded.
MOCAP_MAGIC_FEET = 0x56424732
MOCAP_STRUCT_FEET = struct.Struct("<Iq" + "d" * 20)


# Leg kinematics from plants/go2/go2.xml, MuJoCo order FL, FR, RL, RR. Chain per leg:
# base -> hip body (HIP_XYZ) -> Rx(q_hip) -> thigh body (THIGH_XYZ) -> Ry(q_thigh)
# -> calf body (CALF_XYZ) -> Ry(q_calf) -> foot geom centre (FOOT_XYZ), sphere radius FOOT_R.
...
def quat_mat(q):
    """Rotation matrix of the wxyz quaternion q."""
    w, x, y, z = q
    return np.array([
...
def calf_rots(base_quat, q):
    """World rotation of each calf frame: R_base @ Rx(hip) @ Ry(thigh + calf)."""
    rb = quat_mat(base_quat)
    out = np.empty((4, 3, 3))
    for i in range(4):
        h, a = q[3 * i], q[3 * i + 1] + q[3 * i + 2]
        ch, sh, ca, sa = np.cos(h), np.sin(h), np.cos(a), np.sin(a)
        out[i] = rb @ np.array([[1, 0, 0], [0, ch, -sh], [0, sh, ch]]) @ \
            np.array([[ca, 0, sa], [0, 1, 0], [-sa, 0, ca]])
    return out


def quat_rotate(q, v):
    """Rotates v by the wxyz quaternion q (same convention as x[3:7])."""
    t = 2.0 * np.cross(q[1:], v)
    return v + q[0] * t + np.cross(q[1:], t)


def feet_in_base(q):
    """Foot-geom centres in the base frame from the 12 joint angles (MuJoCo order)."""
    out = np.empty((4, 3))
    for i in range(4):
        ...
        chh, shh = np.cos(h), np.sin(h)
        out[i] = HIP_XYZ[i] + np.array([p[0], chh * p[1] - shh * p[2], shh * p[1] + chh * p[2]])
    return out
...
class Custom:
    def __init__(self):
        ...
        obs_tag = str(np.asarray(ck.get("obs", "")))
        self.use_mocap = obs_tag in ("euler_v3_pos", "euler_v4_feet")
        self.use_feet = obs_tag == "euler_v4_feet"
        obs_width = (73 + 3 * self.use_accelerometer + 3 * self.use_mocap
                     + 4 * self.use_feet + 30 * len(self.preview_offsets))
        if self.arch == "gru":
        ...
        self.foot_clr = np.full(4, np.nan)      # what the actor consumed
        self.foot_clr_fk = np.full(4, np.nan)   # always computed, for the overlay
        self.foot_clr_mocap = np.full(4, np.nan)
        self.foot_delta_calf = None             # marker -> contact offset, calf frame
        self.foot_src = np.zeros(4, np.int8)    # 1 = mocap this tick, 0 = FK fallback
        self.n_foot_fallback = np.zeros(4, np.int64)
        self.log_feet = []
    ...
    def DrainMocap(self):
        ...
        while True:
            ...
            if len(buf) == MOCAP_STRUCT.size:
                ...
                if f[0] != MOCAP_MAGIC:
                    continue
            elif len(buf) == MOCAP_STRUCT_FEET.size:
                f = MOCAP_STRUCT_FEET.unpack(buf)
                if f[0] != MOCAP_MAGIC_FEET:
                    continue
            else:
                continue
            got = f
        ...
        if not np.isfinite(p).all():
            return None
        q = np.array(got[6:10])
        # Feet are NOT gated on finiteness: an occluded foot arrives NaN and falls back to
        # FK per foot, which must not cost us the base sample in the same datagram.
        feet = np.array(got[10:22]).reshape(4, 3) if len(got) == 22 else None
        # perf_counter throughout: ages are compared against this same clock downstream.
        self.mocap_sample = (time.perf_counter(), int(got[1]), p, q, feet)
    ...
    def MocapAnchor(self, p_raw):
        """Raw Vicon metres -> the anchored frame MocapPosition returns (same rigid map)."""
        d = np.asarray(p_raw, float) - self.mocap_zero
        xy = self.mocap_R @ d[:2]
        return self.x_ref[0, :3] + np.array([xy[0], xy[1], d[2]])
    ...
    def LowCmdWrite(self):
        ...
        if self.fold_percent < 1:
            ...
            if self.use_feet:
                # FK path (hopscotch3): p_meas is anchored to the RAW x_ref[0] (point-foot
                # solve) while the trainer raised its reference by the foot radius, so p_meas
                # sits exactly FOOT_R below true world height -- the radius cancels and
                # clr = p_meas_z + (R @ p_foot)_z.
                pf = feet_in_base(dof_pos)
                self.foot_clr_fk = np.array(
                    [self.p_meas[2] + quat_rotate(base_quat, pf[i])[2] for i in range(4)])
                self.foot_clr_mocap = np.full(4, np.nan)
                smp = self.mocap_sample
                feet_raw = smp[4] if (smp is not None and len(smp) > 4) else None
                if feet_raw is not None and self.mocap_zero is not None:
                    rc = calf_rots(base_quat, dof_pos)
                    anch = np.array([self.MocapAnchor(feet_raw[i]) for i in range(4)])
                    if self.foot_delta_calf is None and np.isfinite(feet_raw).all():
                        # One-shot: pin the marker->contact offset in the calf frame using FK
                        # at this tick (still the hold pose). FK's static offset transfers
                        # once; from here the marker supplies the motion, on a ~22 mm lever
                        # instead of FK's ~236 mm -- ~10x less sensitive to attitude error.
                        contact = np.array(
                            [self.p_meas + quat_rotate(base_quat, pf[i]) for i in range(4)])
                        self.foot_delta_calf = np.array(
                            [rc[i].T @ (contact[i] - anch[i]) for i in range(4)])
                        print("foot markers calibrated: |delta| mm "
                              + np.array2string(
                                  1e3 * np.linalg.norm(self.foot_delta_calf, axis=1),
                                  precision=1), flush=True)
                    if self.foot_delta_calf is not None:
                        self.foot_clr_mocap = np.array(
                            [(anch[i] + rc[i] @ self.foot_delta_calf[i])[2] for i in range(4)])
                ok = np.isfinite(self.foot_clr_mocap)
                self.foot_src = ok.astype(np.int8)
                self.n_foot_fallback += ~ok
                self.foot_clr = np.where(ok, self.foot_clr_mocap, self.foot_clr_fk)
                self.log_feet.append(np.concatenate(
                    [[self.ii], self.foot_clr, self.foot_clr_fk, self.foot_clr_mocap,
                     self.foot_src]))
                obs.append(self.foot_clr)
    ...
    while True:
        if custom.settle_percent >= 1:
           ...
           np.savez("run_log.npz", tick=np.array(custom.log_tick),
                    ...
                    feet=np.array(custom.log_feet),
                    feet_layout="ii,clr_used4,clr_fk4,clr_mocap4,src4",
                    layout="ii,q12,dq12,quat4,gyro3,v12,tau_meas12,tau_cmd12,accel3,foot4,"
                           "mocap3,mocap_raw3,mocap_age1,mocap_seq1")
           if len(custom.log_feet):
               print("foot clearance source: mocap "
                     + ", ".join(f"{n} {100.0 * (1 - custom.n_foot_fallback[i] / len(custom.log_feet)):.0f}%"
