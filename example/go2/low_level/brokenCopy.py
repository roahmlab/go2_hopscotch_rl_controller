	...
	def LowCmdWrite(self):
    	...
    	if self.alignment_percent < 0.5:
        	...
        	for i in range(12):
            	...
            	self.low_cmd.motor_cmd[idx].q = float((1-2*self.alignment_percent) * self.startPos[i] + 2*self.alignment_percent * self.q0[i])
            	self.low_cmd.motor_cmd[idx].dq = float((self.q0[i] - self.startPos[i]) / 2.5)
        	...
        	logging.info("inf norm error: " + str(np.abs(dof_pos - self.q0)))
   	 
    	elif self.alignment_percent >= 0.5 and self.alignment_percent < 1:
