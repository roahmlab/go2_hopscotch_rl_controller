...
def run_sysid(csv_path="./exciting-trajectory-2.csv", iface=None):
    ...
    try:
        while True:
            ...
            tau_estimated = robot.get_tau_est() ## estimated, but coming from go2
            ...
            log['tau_est'].append(tau_estimated) ### from robot
            log['q_ref'].append(q3)
            log['qd_ref'].append(qd3)
            log['qdd_ref'].append(qdd3)
            log['tau_ff'].append(tau3) # what we gave
            log['tau_fb'].append(tau_fb) ### what should be calculated
        ...
        print(f"Saved {len(log['t'])} samples to {traj_path} (+ debug: {debug_path})")
         ...
         # --- Friction candidate score: RMS of feedback effort (lower = better model) ---
        ...
        rms_move = np.sqrt(np.array([np.mean(tau_fb_arr[moving[:, j], j] ** 2) for j in range(3)]))
...
if __name__ == '__main__':
    ...
    ''' 
    usage: python deploy_reference_traj.py <iface> ./reference_FR_cand0.csv
      python deploy_reference_traj.py <iface> ./reference_FR_cand1.csv  
    '''