...
def run_sysid(csv_path="./reference_FR_urdf.csv", iface=None):
   ...
   log = {'t': [], 'q_actual': [], 'qd_actual': [], 'qdd_actual': [], 'tau_est': [],
         'q_ref': [], 'qd_ref': [], 'qdd_ref': [], 'tau_ff': [], 'tau_fb': [], 'q_err': []}
   ...
   try:
      while True:
         ...
         ## Calculate acceleration by differentiating velocity data (move to 4rth order )
         ...
         tau_fb = KP_ACTIVE * (q3 - q_actual) + KD_ACTIVE * (qd3 - qd_actual) ### residual: what PD had to add on top of tau_ff
         q_err = q3 - q_actual ### tracking error: ref minus actual
      ...
      out_dir = os.path.abspath(os.path.join(script_dir, "../../data/go2/FR/inertia"))
      ...
      # Secondary file: ref/feedforward/feedback/tracking-error diagnostics, not consumed by the pipeline
      debug_path = f"{out_dir}/traj_data_{timestamp}_debug.csv"
      debug_cols = np.hstack([np.array(log[k]) for k in ('q_ref', 'qd_ref', 'qdd_ref', 'tau_ff', 'tau_fb', 'q_err')])
      np.savetxt(debug_path, np.hstack([t_col, debug_cols]))

      print(f"Saved {len(log['t'])} samples to {traj_path} (+ debug: {debug_path})")

      tau_fb_arr = np.array(log['tau_fb'])            # (N, 3) torque residual
      q_err_arr  = np.array(log['q_err'])             # (N, 3) tracking error
      qd_ref_arr = np.array(log['qd_ref'])            # (N, 3)

      rms_tau_all   = np.sqrt(np.mean(tau_fb_arr ** 2, axis=0))
      rms_qerr_all  = np.sqrt(np.mean(q_err_arr ** 2, axis=0))

      moving = np.abs(qd_ref_arr) >= 0.05             # mask near-zero-vel (ramp/reversals)
      rms_tau_move = np.sqrt(np.array(
         [np.mean(tau_fb_arr[moving[:, j], j] ** 2) for j in range(3)]))
      rms_qerr_move = np.sqrt(np.array(
         [np.mean(q_err_arr[moving[:, j], j] ** 2) for j in range(3)]))

      print(f"[score] RMS tracking error |q-qref| per joint (all)    = {rms_qerr_all}")
      print(f"[score] RMS tracking error |q-qref| per joint (moving) = {rms_qerr_move}")
      print(f"[score] RMS torque residual |tau_fb| per joint (all)    = {rms_tau_all}")
      print(f"[score] RMS torque residual |tau_fb| per joint (moving) = {rms_tau_move}   <- rank on this")

      summary_path = f"{out_dir}/inertial_candidate_scores.csv"
      with open(summary_path, 'a') as f:
         f.write(f"{timestamp} {os.path.basename(csv_path)} "
                  f"{rms_qerr_move[0]:.6f} {rms_qerr_move[1]:.6f} {rms_qerr_move[2]:.6f} {np.linalg.norm(rms_qerr_move):.6f} "
                  f"{rms_tau_move[0]:.6f} {rms_tau_move[1]:.6f} {rms_tau_move[2]:.6f} {np.linalg.norm(rms_tau_move):.6f}\n")
...
if __name__ == '__main__':
   ...
   csv_path = sys.argv[2] if len(sys.argv) > 2 else "./reference_FR_urdf.csv"
